...
def connect_to_serial_port(open_ports):
    try:  # port configuration
        ...
        print "Could not open a active serial port ttyUSB0."
        print "Please check that the USB devices are connected and powered up"
...
class Dispenser:
    ...
    command = {
        ...
        'write_hold_card': "02018701000386",#checkbox off
        'read_hold_card': "020188000388",
        'write_delay_time': "0201890102038A",  # number 0 causes stuck card if not zero
    ...
    status_response = {
        ...
        '02010601340333': 'card_hold',  # wait time until ready again should be 0
    ...
    def init(self,ser):
        # stuck card problem if not zero
        try:
            if ser.isOpen:
                ser.write(self.command['write_hold_card'].decode('hex'))
                ser.close()
                return True
    ...
    def request_status(self,ser):
        try:
            if ser.isOpen:
                ser.write(self.command['request_status'].decode('hex'))
                response_received = ser.readline(7).encode("hex")
                ser.close()
                # check if response is valid status response
                if response_received in self.status_response:
                    return (self.status_response[response_received])
                else:
                    #TODO debug me
                    return ("error {}".format(response_received))
    ...
    def is_busy(self,ser):
        # check if status response is busy
        if self.request_status(ser) == 'busy':
            return True
        else:
            return False

    def is_empty(self,ser):
        # check if status response is empty
        if self.request_status(ser) == 'empty':
            return True
        else:
            return False

    def is_ready(self,ser):
        # check if status response is ready
        if self. request_status(ser) == 'ready':
            return True
        else:
            return False

    def is_card_hold(self,ser):
        # check if status response is card hold
        if self.request_status(ser) == 'card_hold':
            return True
        else:
            return False

    def is_partial_dispense(self,ser):
        # check if status response is partial dispense
        if self.request_status(ser) == 'partial_dispense':
            return True
        else:
            return False

    def dispense(self,ser):
        try:
            if ser.isOpen:
                ser.write(self.command['dispense'].decode('hex'))
                ser.close()
                return True
    ...
    def reset_card_dispenser(self,ser):
        try:
            if ser.isOpen:
                ser.write(self.command['reset_card_dispenser'].decode('hex'))
                ser.close()
                return True
...
if __name__ == "__main__":
    ...
    test = Dispenser(connect_to_serial_port('/dev/ttyUSB0'))
    test.request_status(connect_to_serial_port('/dev/ttyUSB0'))

    print "dispenser test complete !"